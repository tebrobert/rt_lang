...
def lexx_idf(code_ext, tokens, token_idx_end):
    ...
    return (code_ext, idx_idf_end, tokens + [Token_Idf(code_ext[idx_idf_start:idx_idf_end])])
...
def lexx_eq_gr(code_ext, tokens, token_idx_end):
    return ((code_ext, token_idx_end + 2, tokens + [Token_Eq_Gr()])
        ...
        else fail(LexxErr(f'Unexpected sequence "={code_ext[token_idx_end + 1]}"'))
    )
...
def lexx_string(code_ext, tokens, token_idx_end):
    ...
    return (code_ext, idx_string_end + 1, tokens + [Token_Lit_Str(code_ext[idx_string_start:idx_string_end])])
...
def lexx_rec(code_ext, token_idx_end, tokens):
    ...
    return (
        ...
        rec(*lexx_idf(code_ext, tokens, token_idx_end))
            if 'a' <= current_char <= 'z' or 'A' <= current_char <= 'Z' or current_char == "_" else
