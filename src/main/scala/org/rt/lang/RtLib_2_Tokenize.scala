...
object RtLib_2_Tokenize {
  ...
  private def rt_assert_token_idf(token: Tok) =
    match_token(
      case_idf = Some(_ => ()),
  ...
  private def lexx_idf(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
  ...
  private def lexx_integer(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
  ...
  private def lexx_paren_open(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext(current_idx) == '(')
    (code_ext, current_idx + 1, tokens.appended(TokParenOpen))
  }

  private def lexx_paren_close(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext(current_idx) == ')')
    (code_ext, current_idx + 1, tokens.appended(TokParenClose))
  }

  private def lexx_eq_gr(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext.substring(current_idx).startsWith("=>"))
    (code_ext, current_idx + 2, tokens.appended(TokEqGr))
  }

  private def lexx_less_minus(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext.substring(current_idx).startsWith("<-"))
    (code_ext, current_idx + 2, tokens.appended(TokLessMinus))
  }

  private def lexx_eq(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext.substring(current_idx).startsWith("="))
    rt_assert(!is_operator_char(code_ext(current_idx + 1)))
    (code_ext, current_idx + 1, tokens.appended(TokEq))
  }

  private def lexx_endl(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext.substring(current_idx).startsWith("\n"))
    (code_ext, current_idx + 1, tokens.appended(TokEndl))
  }

  private def lexx_dot(code_ext: String, current_idx: Int, tokens: List[Tok]) = {
    rt_assert(code_ext(current_idx) == '.')
    (code_ext, current_idx + 1, tokens.appended(TokDot))
  }

  private def lexx_string(code_ext: String, token_idx_end: Int, tokens: List[Tok]) = {
  ...
  private def lexx_operator(code_ext: String, token_idx_end: Int, tokens: List[Tok]) = {
