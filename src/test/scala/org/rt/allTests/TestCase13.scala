...
object TestCase13 extends RtTestCase {
  ...
  val expr_2 =
    exprEqAndThen(
      ...
      exprAndThen(
        "_",
        ExprCall1(ExprIdf(vPrint), ExprIdf("greeting")),
        exprAndThen(
          "name",
          ExprIdf(vInput),
          exprEqAndThen(
            "result",
            exprCalls(ExprIdf(vPlus), ExprIdf("name"), ExprLitStr("Welcome, ")),
    ...
    Right(
      lintedEqAndThen(
        ...
        lintedAndThen(
          "_",
          LintedCall1(
            LintedIdf(vPrint, T_Str_To_RIO_Unit),
            LintedIdf("greeting", T_Str),
            T_RIO_Unit,
          ),
          lintedAndThen(
            "name",
            LintedIdf(vInput, T_RIO_Str),
            lintedEqAndThen(
              "result",
              lintedCalls(
                LintedIdf(vPlus, T_Str_To_Str_To_Str),
                LintedIdf("name", T_Str),
                LintedLit("Welcome, ", T_Str),
              ),
              LintedCall1(
                LintedIdf(vPrint, T_Str_To_RIO_Unit),
                LintedIdf("result", T_Str),
                T_RIO_Unit,
              ),
            ),
          ),
